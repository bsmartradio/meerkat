    ...
    for single_channel in channels:
        for chan_num in range(9):
            if any(x in single_channel for x in ["chan" + "{:01d}".format(chan_num + 1) + '.']):
                logging.info('File name needs to be changed for file: ' + single_channel)
...
def process_channels_check(location, channels, total_channels, backgrounds_list, phot_lookup=True):
    ...
    for k in range(total_channels):
        if not phot_exist:
            ...
            channel_exists_check = channels[k].find("chan" + "{:02d}".format(k + 1) + '.')
            logging.info("Does chan" + "{:02d}".format(k + 1) + ' exist?')
            ...
            if channel_exists_check:
                ...
                channel_data = helper.get_image(channels[k])
                check_values = np.isnan(channel_data[0][:, :]).all()
                bck_file = [s for s in backgrounds_list if "chan" + "{:02d}".format(k + 1) + "_bkg" in s]

                if not check_values and bck_file != " ":
                    existing_channels.append(k)
                    logging.info(f'Channel {k + 1} has values and valid background, will process photometry')

                elif bck_file == " " and check_values:
                    logging.warning(f'Missing background file for {k + 1} channel. Run auto_bane to make file.')
                    logging.warning(f"Skipping file {location} due to missing files")
                elif bck_file != " " and check_values:
                    logging.warning(f'No values for {k + 1} channel')
                else:
                    logging.warning(f'No values for {k + 1} channel and missing background file. '
                                    f'Run auto_bane to make file.')
            else:
                logging.warning('Channel does not exist. Please check if all fits files are present.')

        elif phot_exist and k == 13 and len(existing_channels) == 0:
            logging.info(
