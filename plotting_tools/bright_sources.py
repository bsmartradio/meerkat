...
def plot(cubes, photometry_tables, show=True, save = False):
    ...
    for channel in range(14):
        ...
        for im in range(3, 0, -1):
            bright = np.where(photometry_tables[im - 1]['si_m'] > .1)
            test = np.where(photometry_tables[im - 1]['overlap'] >= 2)
            ...
            axarr[3 - im].scatter(cubes[im - 1].positions[test, 0], cubes[im - 1].positions[test, 1], s=1)
