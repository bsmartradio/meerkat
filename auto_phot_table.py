...
import common.data_helper as meer


# This program receives a folder location and filename of a MeerKAT data cube.
# Using an existing Aegean point source list, it processes each
# point source using photutils aperture photometry, correcting for
# beam size. The program is intended to run on machines with at least 32 cores avaliable
# for multiprocessing to reduce the length of time it takes to process.
# what needs to be done in this program
#  needs to read in a cube (possibly a list of cubes)
# It then needs to plot the cube and the respective source positions (possibly to a file)
# It then needs to take the source positions, translate them into pixel coordinates
    ...
    if phot_exist == False:
        ...
        if chan_check != False:
            ...
            if check == False and bck_file != " ":
                ...
                print('Will processing apertures')
...
if __name__ == "__main__":
    ...
    if all_lists_check == True:
        ...
        app_list = np.full([total_channels, shape], float('Nan'))
        err_list = np.full([total_channels, shape], float('Nan'))
        # List of frequencies
        freq_list = np.full([total_channels], float('Nan'))
        count = 0
        name = meer.get_name(location)
        # name=location[last_char_index+1:-1]
